...
export default function BookingCalendar() {
  ...
  const [horasOcupadas, setHorasOcupadas] = useState<string[]>([]); // ✅ almacena horas reservadas
  ...
  // ✅ Función para cargar las horas ya ocupadas en la fecha seleccionada
  async function cargarHorasOcupadas(fecha: Date) {
    const fechaStr = fecha.toLocaleDateString(); // mismo formato que guardamos
    ...
    try {
      ...
      const ocupadas = data.map((reserva: any) => reserva.hora);
  ...
  // ✅ Cada vez que cambia la fecha seleccionada, recargamos las horas ocupadas
  useEffect(() => {
    if (selected) {
      cargarHorasOcupadas(selected);
      // También reiniciamos la hora seleccionada por si la fecha cambió
      setHora("");
    }
  }, [selected]);
  ...
  async function guardarReserva() {
    ...
    // ✅ Verificación adicional por si el horario se ocupó mientras elegías otros datos
    if (estaOcupado(hora)) {
      alert("Lo sentimos, este horario ya fue reservado. Por favor elige otro.");
      setHora(""); // limpiar selección
      return;
    }
